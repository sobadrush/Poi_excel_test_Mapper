...
//	@Ignore
	@Rollback(true)
	public void test003() throws SQLException, IOException {
		System.out.println("============== test003 >>> 取得POI workbook 的 inputStream (自訂sheets) ==============");
		System.out.println("============== test003 >>> 取得POI workbook 的 inputStream (自訂sheets) ==============");
		System.out.println("============== test003 >>> 取得POI workbook 的 inputStream (自訂sheets) ==============");
		
		// (1) 產製Excel檔案
		InputStream excelInputStream = excelTemplate.doCustomerExcelGetInputStream(new CustomizeExceler() {
			...
			protected void customerExcel(Class<?> voClazz) {
				...
				while (sheetNum < numberOfSheet) {
					...
					if (sheetNum == 0) {
						int titleStartRow = rownum_start - 1;
						ExcelUtil.createTitleRow(this.getSheets()[sheetNum], 
								new String[] { "員工編號", "到職日", "姓名", "職位", "部門編號", "部門名稱" }, titleCellStyle, titleStartRow, colnum_title);
					} // [標題列]-END
...
//							excelCell02.setCellValue(sdf.format(empVO.getEmpHireDate()));// 到職日
		...
		}, null, new String[] { "員工清單001", "員工清單002" });// end-of-excelTemplate
		
		// (2) 寫檔案
		String outputFilePath = "D:/excelSample/EmpData_003.xlsx"; 
		FileUtils.writeByteArrayToFile(new File(outputFilePath), IOUtils.toByteArray(excelInputStream));
		System.out.println(" === 寫出檔案成功！( " + outputFilePath + " ) === ");
